...
send_stats() {
	...
	(
		curl -s -X POST "https://api.shabe.com/api/log" \
...
# 清理旧的xl别名（若存在）
sed -i '/^alias xl=/d' ~/.bashrc > /dev/null 2>&1
sed -i '/^alias xl=/d' ~/.profile > /dev/null 2>&1
sed -i '/^alias xl=/d' ~/.bash_profile > /dev/null 2>&1
# 备份并部署xl命令
cp -f ./kejilion.sh ~/kejilion.sh > /dev/null 2>&1
cp -f ~/kejilion.sh /usr/local/bin/xl > /dev/null 2>&1



CheckFirstRun_false() {
	if grep -q '^permission_granted="false"' /usr/local/bin/xl > /dev/null 2>&1; then
...
UserLicenseAgreement() {
	...
	echo "用户许可协议: https://blog.shabe.com/user-license-agreement/"
	echo -e "----------------------"
	...
	if [ "$user_input" = "y" ] || [ "$user_input" = "Y" ]; then
		send_stats "许可同意"
		sed -i 's/^permission_granted="false"/permission_granted="true"/' ~/kejilion.sh
		sed -i 's/^permission_granted="false"/permission_granted="true"/' /usr/local/bin/xl
	else
		send_stats "许可拒绝"
		clear
		exit
...
if [ "$country" = "CN" ]; then
	cd ~
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/liluosheng/VPS/main/Docker/install && chmod +x install
...
install_ldnmp_conf() {
  ...
  # 创建必要的目录和文件
  cd /home && mkdir -p XL/website/html XL/website/mysql XL/websitecerts XL/website/conf.d XL/website/redis XL/websitelog/nginx && touch XL/website/docker-compose.yml
  wget -O /home/web/nginx.conf ${gh_proxy}raw.githubusercontent.com/liluosheng/VPS/main/Nginx/nginx10.conf
  wget -O /home/web/conf.d/default.conf ${gh_proxy}raw.githubusercontent.com/liluosheng/VPS/main/Nginx/default10.conf
  wget -O /home/web/redis/valkey.conf ${gh_proxy}raw.githubusercontent.com/liluosheng/VPS/main/sh/valkey.conf


  default_server_ssl

  # 下载 docker-compose.yml 文件并进行替换
  wget -O /home/web/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/liluosheng/VPS/main/Docker/LNMP-docker-compose-10.yml
...
install_certbot() {
	...
	cd ~
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/liluosheng/VPS/sh/main/auto_cert_renewal.sh
...
echo "以下是xl命令参考用例："
echo "启动脚本            xl"
echo "安装软件包          xl install nano wget | xl add nano wget | xl 安装 nano wget"
echo "卸载软件包          xl remove nano wget | xl del nano wget | xl uninstall nano wget | xl 卸载 nano wget"
echo "更新系统            xl update | xl 更新"
echo "清理系统垃圾        xl clean | xl 清理"
echo "重装系统面板        xl dd | xl 重装"
echo "bbr3控制面板        xl bbr3 | xl bbrv3"
echo "内核调优面板        xl nhyh | xl 内核优化"
echo "设置虚拟内存        xl swap 2048"
echo "设置虚拟时区        xl time Asia/Shanghai | xl 时区 Asia/Shanghai"
echo "系统回收站          xl trash | xl hsz | xl 回收站"
echo "系统备份功能        xl backup | xl bf | xl 备份"
echo "ssh远程连接工具     xl ssh | xl 远程连接"
echo "rsync远程同步工具   xl rsync | xl 远程同步"
echo "硬盘管理工具        xl disk | xl 硬盘管理"
echo "内网穿透（服务端）  xl frps"
echo "内网穿透（客户端）  xl frpc"
echo "软件启动            xl start sshd | xl 启动 sshd "
echo "软件停止            xl stop sshd | xl 停止 sshd "
echo "软件重启            xl restart sshd | xl 重启 sshd "
echo "软件状态查看        xl status sshd | xl 状态 sshd "
echo "软件开机启动        xl enable docker | xl autostart docke | xl 开机启动 docker "
echo "域名证书申请        xl ssl"
echo "域名证书到期查询    xl ssl ps"
echo "docker管理平面      xl docker"
echo "docker环境安装      xl docker install |xl docker 安装"
echo "docker容器管理      xl docker ps |xl docker 容器"
echo "docker镜像管理      xl docker img |xl docker 镜像"
echo "LDNMP站点管理       xl web"
echo "LDNMP缓存清理       xl web cache"
echo "安装WordPress       xl wp |xl wordpress |xl wp xxx.com"
echo "安装反向代理        xl fd |xl rp |xl 反代 |xl fd xxx.com"
echo "安装负载均衡        xl loadbalance |xl 负载均衡"
echo "防火墙面板          xl fhq |xl 防火墙"
echo "开放端口            xl dkdk 8080 |xl 打开端口 8080"
echo "关闭端口            xl gbdk 7800 |xl 关闭端口 7800"
echo "放行IP              xl fxip 127.0.0.0/8 |xl 放行IP 127.0.0.0/8"
echo "阻止IP              xl zzip 177.5.25.36 |xl 阻止IP 177.5.25.36"
echo "命令收藏夹          xl fav | xl 命令收藏夹"
echo "应用市场管理        xl app"
echo "应用编号快捷管理    xl app 26 | xl app 1panel | xl app npm"
echo "显示系统信息        xl info"
}
